...
export default function CiderManager() {
  ...
  const buildInSiteScript = (): string => {
    ...
    return wrapInSiteScript('Cider', body, enabledFeatures, '#8b5cf6');
  };
  ...
  const handleCopyScript = async () => {
    ...
    try {
      ...
      if (result.success) {
        toast.success('Script de injeção Cider copiado!', {
      ...
      console.error(e);
      toast.error('Erro ao copiar o script de injeção Cider');
    } finally {
  ...
  const handleOpenSite = () => {
    openSiteInNewTab('https://www.shopcider.com');
    toast.info('Cider aberta em nova guia', {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid gap-6 mt-8">
          ...
          <div className="border border-violet-600/30 rounded-2xl p-6 bg-card/50 backdrop-blur-sm shadow-xl">
            ...
            <div className="mt-8 pt-6 border-t border-violet-600/20">
              <div className="flex flex-col sm:flex-row gap-4 items-center justify-between mb-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                  <Button
                    ...
                    disabled={!device}
                    className="w-full sm:w-auto bg-transparent hover:bg-violet-600/20 text-violet-400 border border-violet-500/60 font-bold px-6 py-3 rounded-xl flex items-center justify-center gap-2"
                  >
              ...
              <div className="rounded-xl bg-background/50 border border-violet-600/20 p-4 text-xs font-mono space-y-1.5">
                <p className="text-violet-400 font-bold mb-2 flex items-center gap-2">
                  <TerminalSquare className="w-4 h-4" />
                  COMO INJETAR NO SITE REAL (Console)
                </p>
                {IN_SITE_STEPS.map((step, i) => (
                  <p key={i} className="text-violet-200/80">
                    <span className="text-violet-400 font-bold">{i + 1}.</span> {step}
                  </p>
                ))}
                <p className="text-yellow-400/90 mt-2 pt-2 border-t border-violet-600/20">
                  ⚠️ O script roda no domínio da Cider real (shopcider.com). O perfil é gravado no localStorage/cookies DESTE domínio — sem aba intermediária, sem redirect.
                </p>
              </div>
            </div>
          </div>
